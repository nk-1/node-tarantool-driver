...
TarantoolConnection.prototype.delete = function(spaceId, indexId, key){
    ...
    return new Promise(function (resolve, reject) {
        if (Array.isArray(key))
        {
            ...
            this.socket.write(buffer);
            this.commandsQueue.push([tarantoolConstants.RequestCode.rqDelete, reqId, {resolve: resolve, reject: reject}]);
        }
        else
...
TarantoolConnection.prototype.eval = function(expression){
    ...
    return new Promise(function (resolve, reject) {
        ...
        var buffer = createBuffer(len+2);

        buffer[0] = 0xce;
        buffer.writeUInt32BE(len-3, 1);
        buffer[5] = 0x82;
        buffer[6] = tarantoolConstants.KeysCode.code;
